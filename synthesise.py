...
def find_transcript_v2(
    transcript_root: Path, session_id: str, speaker: str
) -> Path | None:
...
def find_transcript_clarin(
    corpus_root: Path, session_id: str, speaker: str
) -> Path | None:
...
def aligned_output_name(
    tx_path: Path, session_id: str, speaker: str, source: str
) -> str:
    if source == "v2":
...
def process_session(
    ...
    transcript_source = (
        "v2" if (tx_ref_source == "v2" and tx_tgt_source == "v2") else "clarin"
    )
    ...
    if n_tgt != n_ref:
        print(
            f"  [{session_id}] Resampling channel {target_channel} (ratio {resample_ratio:.6f})..."
        )
        sr_scaled = round(sr_tgt * resample_ratio)
        y_tgt_aligned = librosa.resample(
            y_tgt, orig_sr=sr_tgt, target_sr=sr_scaled, res_type=res_type
        )
    ...
    ref_tx_out = output_dir / aligned_output_name(
        tx_ref_path, session_id, reference_channel, tx_ref_source
    )
    tgt_tx_out = output_dir / aligned_output_name(
        tx_tgt_path, session_id, target_channel, tx_tgt_source
    )
    ...
    # Merged transcript.
    all_words = list(tx_ref_aligned.get("words", [])) + list(
        tx_tgt_aligned.get("words", [])
    )
...
def main():
    ...
    parser.add_argument(
        "--corpus-root", required=True, help="Path to CLARIN corpus root (WAV files)."
    )
    parser.add_argument(
        "--transcript-root",
        required=True,
        help="Path to v2 transcript root; CLARIN transcripts used as fallback when v2 not found.",
    )
    parser.add_argument("--output-root", required=True, help="Path to output root.")
    parser.add_argument(
        "--sessions", default=None, help="Comma-separated session IDs (default: all)."
    )
    parser.add_argument(
        "--res-type", default="kaiser_best", help="librosa resampling quality."
    )
    ...
    for param_path in param_files:
        ...
        if params.get("status") == "excluded":
            print(
                f"[{session_id}] Skipping (excluded: {params.get('exclusion_reason', '')})"
            )
            skipped += 1
            continue
