...
The manual transcription coverage stats are read from
  <corpus-root>/.../docs/manual_transcripts.json
which the corpus is expected to supply already anonymised.
"""
...
def find_manual_transcripts_file(corpus_root: Path) -> Path | None:
    ...
    anonymised.  manual_transcripts_original.json, where a corpus copy retains
    it, is deliberately not matched by this exact-name lookup.
    """
    return next(corpus_root.rglob("manual_transcripts.json"), None)
